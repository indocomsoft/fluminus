defmodule Fluminus.AuthorizationTest do
  ...
  test "jwt errors", %{bypass: bypass} do
    {:error, :invalid_credentials} = Authorization.jwt("e1234567", "wrongpassword")

    Bypass.down(bypass)
    {:error, %HTTPoison.Error{reason: :econnrefused}} = Authorization.jwt("e0123456", "hunter2")
  end
  ...
  defp handle_conn(conn) do
    ...
    if cassette do
      %{"response" => %{"headers" => headers, "status_code" => status_code, "body" => body}} = cassette
